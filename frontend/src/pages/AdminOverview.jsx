...
export default function AdminOverview() {
  ...
  return (
    <section>
      ...
      <p className="mt-10 text-xs font-semibold uppercase tracking-[0.2em] text-brand-primary">AI chatbot</p>
      <div className="mt-3 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <article className="rounded-2xl border border-black/10 bg-white p-5 shadow-[0_10px_30px_rgba(0,0,0,0.08)]">
          <p className="text-xs uppercase tracking-[0.18em] text-brand-text/60">Tagged AI replies</p>
          <p className="mt-2 text-3xl font-semibold text-brand-text">{data?.chatbot?.tagged_replies ?? 0}</p>
        </article>
        <article className="rounded-2xl border border-black/10 bg-white p-5 shadow-[0_10px_30px_rgba(0,0,0,0.08)]">
          <p className="text-xs uppercase tracking-[0.18em] text-brand-text/60">Uncertain replies</p>
          <p className="mt-2 text-3xl font-semibold text-brand-text">{data?.chatbot?.uncertain_replies ?? 0}</p>
        </article>
        <article className="rounded-2xl border border-black/10 bg-white p-5 shadow-[0_10px_30px_rgba(0,0,0,0.08)]">
          <p className="text-xs uppercase tracking-[0.18em] text-brand-text/60">Escalations</p>
          <p className="mt-2 text-3xl font-semibold text-brand-text">{data?.chatbot?.escalation_events ?? 0}</p>
        </article>
        <article className="rounded-2xl border border-black/10 bg-white p-5 shadow-[0_10px_30px_rgba(0,0,0,0.08)]">
          <p className="text-xs uppercase tracking-[0.18em] text-brand-text/60">Avg. satisfaction (1–5)</p>
          <p className="mt-2 text-3xl font-semibold text-brand-text">{data?.chatbot?.satisfaction_avg ?? '—'}</p>
        </article>
      </div>
    </section>
  )
