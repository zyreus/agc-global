...
class AdminCrmController extends Controller
{
    ...
    public function reply(Request $request, string $sessionId): JsonResponse
    {
        ...
        // Reuse AI endpoint by simulating a user message to generate an assistant reply
        // (but don't store the simulated user message)
        $ai = app(AiChatController::class);
        $ref = (new \ReflectionClass($ai))->getMethod('generateReply');
        $ref->setAccessible(true);
        $assistantReply = (string) $ref->invoke($ai, $sessionId, $prompt);
